package io.owlstreet.product.web.rest;
...
import io.owlstreet.product.IntegrationTest;
import io.owlstreet.product.config.EmbeddedKafka;
import io.owlstreet.product.config.KafkaSseConsumer;
import io.owlstreet.product.config.KafkaSseProducer;
...
@WithMockUser
@EmbeddedKafka
class OwlstreetproductKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/owlstreetproduct-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/owlstreetproduct-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/owlstreetproduct-kafka/unregister"));
                return;
            }
        }
